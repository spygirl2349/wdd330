...
//quarter circle function example
c.fillStyle = 'white';
const drawQuarterCircle = (origin, radius, startAngleDeg = 0) => {
	const startAngle = startAngleDeg * (Math.PI / 180); // 90 * (Math.PI/ 180)
	const endAngle = startAngle + Math.PI * 0.5; // 90 * (Math.PI/ 180) + Math.PI * 0.5
	c.beginPath();
...
drawQuarterCircle({x: 300,y: 101}, 15, 0);
